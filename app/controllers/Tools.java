...
public class Tools {
 ...
 public static  ArrayList<String>  getWeatherCondition(String dbHost, int port, String dbName) {
          ArrayList<String> result = new ArrayList<String>();
          MongoClient mongoClient = new MongoClient(dbHost , port);
          MongoDatabase database = mongoClient.getDatabase(dbName);
          MongoCursor<Document> cursor = database
                .getCollection("weather") // gets the collection (case sensitive)
            ...
            try{
                     ...
                     result.add( cursorNext.get("main").toString());
                     result.add ( cursorNext.get("description").toString());
                     result.add( cursorNext.get("icon").toString());
            } catch (Exception e) {
 ...
 public static List<List<String>> jsonToDataList(String dbHost, int port, String dbName) {
        ...
        String[] COLLECTIONS = {
                        "temperature",
                        "opm_001_Temp_aggregated",
                        "humidity",
                        "opm_002_Humi_aggregated",
                        "windS",
                        "opm_003_WindSpeed_aggregated",
                        "windD",
                        "opm_004_WindDirection_aggregated",
        };
        ...
        for (i = 0; i < COLLECTIONS.length; i++) { // cycle through every collection
            ...
            dataList.add(list);
            long start = System.currentTimeMillis()-(7*24*60*60*1000);
            // create a cursor that cycle through the documents
            MongoCursor<Document> cursor = database
                .getCollection(COLLECTIONS[i]) // gets the collection (case sensitive)
                .find(new BasicDBObject("timestamp", new BasicDBObject("$gte",start))) // return everything is to above collection
                .projection(excludeId()) //exclude the id field in the response
                .sort(descending("timestamp")) // latest data will be at the top
                .iterator(); // give the possibility to the cursor to iterate through the result
 ...
 public static ArrayList<Object> getAllDayValues(List<List<String>> mongoData, int collectionIndex) {
      ...
      float min = 0,
        ...
        if (collectionIndex == 0) { //raws
           current = Float.parseFloat(mongoData.get(collectionIndex).get(1));
            ...
            while (nextTimestamp >= dayLimitTimestamp) {
                if (i == 0) { // cursor on first entry (init timestamps)
                    ...
                    min = max = Float.parseFloat(mongoData.get(collectionIndex).get(i));
                    average += Float.parseFloat(mongoData.get(collectionIndex).get(i));
                } else if (i%3 == 1) { //cursor on every seconds entries (except the first second)
                    average += Float.parseFloat(mongoData.get(collectionIndex).get(i)); // add value to average
                    nextTimestamp = Long.parseLong(mongoData.get(collectionIndex).get(i+2));
                    if (Float.parseFloat(mongoData.get(collectionIndex).get(i)) > max) { //case where value gt max
                    max = Float.parseFloat(mongoData.get(collectionIndex).get(i)); // replace max with new value
                        maxTime = Long.parseLong(mongoData.get(collectionIndex).get(i-1)); // get time associated with this value
                    } else if (Float.parseFloat(mongoData.get(collectionIndex).get(i)) < min) { // case where value lt min
                        min = Float.parseFloat(mongoData.get(collectionIndex).get(i)); // replace min with new value
...
public static ArrayList<Object> getAllDayValue(List<List<String>> mongoData, int collectionIndex) {
        ArrayList<Object> stats = new ArrayList<Object>();
        long lastTimestamp = 0,
            dayLimitTimestamp = 0,
            nextTimestamp = 0,
 ...
 public static ArrayList<Object> getAllDayValuee(List<List<String>> mongoData, int collectionIndex) {
        ArrayList<Object> stats = new ArrayList<Object>();
        long lastTimestamp = 0,
            dayLimitTimestamp = 0,
            nextTimestamp = 0,
 ...
 public static ArrayList<Object> getAllDayValueee(List<List<String>> mongoData, int collectionIndex) {
        ArrayList<Object> stats = new ArrayList<Object>();
        long lastTimestamp = 0,
            dayLimitTimestamp = 0,
            nextTimestamp = 0,
